...
static tick_t one_net_tick(void);   //so that print_debug can call it
...
int main(int argc, char **argv) 
{
    ...
    if(argc == 2)
    {
        //turn on debugging to given file
        set_debug_level(1);
        set_debug_file(argv[1]);
        set_program_name("click");
        set_program_version("(1.0)");
...
/*!
    \brief returns global tick count

    This function is used to return the global tick count
    and is implemented here so that print_debug can use it.  
    NOT actually for ONE-NET, that one is implemented elsewhere

    \param void

    \return the global tick_t tick counter
*/
static tick_t one_net_tick(void)
{
    tick_t ticks = 0;
    acquire_lock(TICK_SEM);
    ticks = *global_tick_ptr;
    release_lock(TICK_SEM);
