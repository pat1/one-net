...
on_raw_sid_t * one_net_master_get_raw_sid(void)
{
    ...
    #endif
        //
        // there is not SID in data eeprom,  use a default SID
        //
...
#endif
{
   ...
   if(eeprom_master_saved)
   {
      ...
      if(status == ONS_MORE)
      {
          status = one_net_master_init(peer_settings, DFI_EEPROM_ONE_NET_PEER_SETTINGS_SIZE);
      }
      #endif
	  */
  ...
  SInt8 channel)
{
	node_loop_func = &master_eval;
 	...
 	one_net_master_create_network((const on_raw_sid_t * const)raw_sid, &EVAL_KEY);
