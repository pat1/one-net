...
BOOL one_net_client_handle_ack_nack_response(const ona_msg_type_t msg_type,
	...
	ack_nack_payload_t* const ack_nack_payload)
{
	return eval_handle_ack_nack_response(msg_type, payload, payload_len, need_txn_payload,
        retries, src_did, nack_reason, ack_nack_handle, ack_nack_payload);
}
...
static void client_check_user_pins(void)
{
    ...
    {
        if(!send_switch_status(USER_PIN0 ? ONA_ON : ONA_OFF, 0,
          ONE_NET_DEV_UNIT, NULL) == ONS_SUCCESS)
        {
    ...
    {
        if(send_switch_status(USER_PIN1 ? ONA_ON : ONA_OFF, 1,
          ONE_NET_DEV_UNIT, NULL) == ONS_SUCCESS)
        {
    ...
    {
        if(send_switch_status(USER_PIN2 ? ONA_ON : ONA_OFF, 2,
          ONE_NET_DEV_UNIT, NULL) == ONS_SUCCESS)
        {
    ...
    {
        if(send_switch_status(USER_PIN3 ? ONA_ON : ONA_OFF, 3,
          ONE_NET_DEV_UNIT, NULL) == ONS_SUCCESS)
        {
