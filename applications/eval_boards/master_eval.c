...
{    
    ...
    // TODO - UInt8 seems too small - UInt16?  Might need to
    // change some function definitions.  one_net_compute_crc
    // takes a UInt8 length parameter.  Seems far too short.
    // Change this if possible/needed.  For now, call it a UInt8
    // so things match.
    //
    // TODO - What exactly are we supposed to crc over?  Everything
    // or just on_base_param?  And what about peer?  CRC them?
    //
    // one_net_master_init takes a UInt16 for a length.  Seems like
    // a mismatch with the CRC.
    //
    UInt16 init_size_no_clients = sizeof(on_base_param_t) +
      sizeof(on_master_param_t);
    UInt16 nv_param_size = init_size_no_clients + ONE_NET_MASTER_MAX_CLIENTS *
      sizeof(on_client_t);
      
    // TODO - see below "TODO" regarding sizes
    UInt8 compute_crc_size = (UInt8) (nv_param_size -
    ...
    on_base_param->crc = one_net_compute_crc((UInt8 *)on_base_param
      + sizeof(on_base_param->crc), compute_crc_size,
      ON_PARAM_INIT_CRC, ON_PARAM_CRC_ORDER);
      
    one_net_master_init((UInt8*) on_base_param, init_size_no_clients);
    return ONCLI_SUCCESS;
...
{
    return eval_handle_status_msg(raw_pld, msg_class, msg_type, src_unit,
       dst_unit, msg_data, src_addr, nack_reason, ack_nack_handle,
       ack_nack_payload);
} // master_handle_status_msg //


BOOL one_net_master_handle_single_pkt(const UInt8* const raw_pld, const ona_msg_class_t msg_class,
	   ...
	   ack_nack_payload_t* const ack_nack_payload)
{
	return eval_handle_single(raw_pld, msg_class, msg_type, src_unit, dst_unit, msg_data,
        SRC_ADDR, useDefaultHandling, nack_reason, ack_nack_handle, ack_nack_payload);
} // master handle_single_pkt //
...
BOOL one_net_master_handle_ack_nack_response(const ona_msg_type_t msg_type,
	...
	ack_nack_payload_t* const ack_nack_payload)
{
	return eval_handle_ack_nack_response(msg_type, payload, payload_len,
	    need_txn_payload, retries, src_did, nack_reason, ack_nack_handle,
        ack_nack_payload);
}
  ...
  ack_nack_payload_t* const ack_nack_payload)
{
    ...
    print_packet(ONCLI_BLOCK_TXN_STR, block_data, LEN, SRC_ADDR);
    block_data_pos += LEN;    
    block_data += LEN;
    return TRUE;
  ...
  ack_nack_payload_t* const ack_nack_payload)  
{
    return eval_txn_requested(TYPE, SEND, DATA_TYPE, DATA_LEN, DID,
      src_unit, dst_unit, nack_reason, ack_nack_handle, ack_nack_payload);
...
void init_serial_master(void)
{
    ...
    if(eval_load(DFI_ST_ONE_NET_MASTER_SETTINGS, &master_len, &MASTER_PARAM))
    {
        ...
        if(eval_load(DFI_ST_APP_DATA_2, &master_peer_len, &MASTER_PEERS)
          && master_peer_len == MASTER_PEER_SIZE_BYTES
...
#ifdef _PEER
            one_net_memmove(master_peer, MASTER_PEERS, MASTER_PEER_SIZE_BYTES);
#endif
    ...
    // July 28, 2011 - Not seeing anywhere where this gets set after
    // successfully loading from non-volatile memory, so putting it here.
    // TODO - Where did it happen before?  Must have deleted something
    // somewhere.
    ...
    if(mode_type() == AUTO_MODE)
    {
        ...
        if(ont_expired(AUTO_MODE_TIMER))
        {
              ...
              client_count++)
            {
                ...
                // updating auto messages so they correspond to the new
                // "send single" spec and don't send to the peer list.
                // Also, each client will now get a different message and we'll
                // use the send_simple_text_command function.  Client 1 will
                // receive "11", client 2 will receive "22", client 3 will receive
                // "33".
                send_simple_text_command((UInt8*) auto_messages[client_count],
                    0, 0, &dst);
...
{
#ifdef _PEER
    // send switch status to the peer list.  Third argument is
    // irrelevant since 4th is null, so I'll fill it in with ONE_NET_DEV_UNIT.
    // It will be overwritten.
    send_switch_status(user_pin[user_pin_src_unit].old_state,
        user_pin_src_unit, ONE_NET_DEV_UNIT, NULL);
#endif
