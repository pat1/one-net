...
#include "hal_lcd_font_fixedsys.h"
//#include "hal_lcd_font_couriernew.h"
//#include "hal_lcd_font_lucida.h"
#include "hal_lcd_font_quartz.h"
...
//font_t f_data[1] = {{font_couriernew_lookup, font_couriernew, 1, 14, 17, '\0', '\0'}};
//
//font_t f_data[1] = {{0, font_fixedsys, 1, 15, 17, 0x20, 0xff}};
//
//font_t f_data[1] = {{font_lucida_lookup, font_lucida, 1, 14, 17, '\0', '\0'}};
//
//font_t f_data[1] = {{font_lucida_lookup, font_lucida_bold, 1, 12, 17, '\0', '\0'}};
//
//font_t f_data[1] = {{font_quartz12_lookup, font_quartz12, 1, 16, 17, '\0', '\0'}};
//
font_t f_data[3] = {{0, font_fixedsys, 1, 15, 17, 0x20, 0xff},
                    {0, font_quartz, 4, 57, 4, ' ', '?'},
                    {0, font_quartz_small, 2, 29, 8, ' ', '?'}};
//                    {font_quartz12_lookup, font_quartz12, 1, 16, 17, '\0', '\0'}
//                   };
//
//font_t f_data[5] = {{font_quartz_lookup, font_quartz, 4, 60, 4, '\0', '\0'}, 
//                    {font_quartz_lookup, font_quartz_small, 2, 32, 8, '\0', '\0'}, 
//                    {font_lucida_lookup, font_lucida, 1, 14, 17, '\0', '\0'}, 
//                    {font_lucida_lookup, font_lucida_bold, 1, 12, 17, '\0', '\0'},
//                    {font_quartz12_lookup, font_quartz12, 1, 16, 17, '\0', '\0'}};

font_t *font = &f_data[0];


//void halLcdPrintXY_1( char String[], int x, int y, unsigned char TextStyle);
//void halLcdPrint_1( char String[], unsigned char TextStyle);
...
void halLcdInit(void) 
{
  ...
  UCB2CTL0 |= UCMSTSPI + UCSYNCSPI + UCCKPLSPI + UCMSBSPI;    // 3-pin, 8-bit SPI master
...
void halLcdPrint( char String[], unsigned char TextStyle) 
{
  ...
  int  ActualAddress;
  int  temp;
  int  font_height = font->hgth;
  int  font_width = font->wdth;
  char LookUpChar;
  
  ActualAddress = LcdAddress;
  Counter = (LcdAddress & 0x1F) / font_width;  
  i = 0;
  
  while (String[i] != 0)                      // Stop on null character
  { 
    if (font->lookup_table)
        LookUpChar = font->lookup_table[String[i]];
    else
        if (String[i] < font->frst_char)
           LookUpChar = 0;
        else
            if (String[i] > font->lst_char)
               LookUpChar = font->lst_char - font->frst_char;
            else
               LookUpChar = String[i] - font->frst_char;
      ...
      for (k = 0; k < font_width ; k++)      
      {
        ...
        if (TextStyle & INVERT_TEXT)         
        {
          if (TextStyle & OVERWRITE_TEXT)
            BlockValue = 0xFFFF - font->font_data[LookUpChar * (font_height + 1) * font_width + j * font_width + k]; 
          else
            BlockValue |= 0xFFFF - font->font_data[LookUpChar * (font_height + 1) * font_width + j * font_width + k]; 
        }  
        else     
        {
          if (TextStyle & OVERWRITE_TEXT)
            BlockValue = font->font_data[LookUpChar * (font_height + 1) * font_width + j * font_width + k]; 
          else
            BlockValue |= font->font_data[LookUpChar * (font_height + 1) * font_width + j * font_width + k]; 
        }  
    ...
    Counter++;
    if (Counter == font->crs_per_line)
    {
      Counter = 0;        
      ActualAddress += 0x20 * font_height - (font->crs_per_line - 1) * font_width;
      if (ActualAddress > LCD_Last_Pixel - 0x20 * font_height)
        ActualAddress = 0;
    }
    else 
      ActualAddress += font_width;
    i++;
  }
...
//  Row = Line * FONT_HEIGHT;
  ...
  for (i = Row; i < Row + font->hgth ; i++)
//  for (i = Row; i < Row + FONT_HEIGHT ; i++)
    halLcdScrollRow(i);
}

void halLcdSelectFont(char idxFont)
{
    font = &f_data[idxFont];
}
...
void halLcdPrintXY_1( char String[], int x, int y, unsigned char TextStyle)  
{
  //Each line increments by 0x20
  halLcdSetAddress( (y << 5) + (x >> 3)) ;  //Narrow down to 8 possible pixels    
  halLcdPrint_1(String,  TextStyle);
}
