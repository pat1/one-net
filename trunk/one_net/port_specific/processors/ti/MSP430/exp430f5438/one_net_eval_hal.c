...
extern BOOL master_get_peer_assignment_to_save(UInt8 **PTR,
  UInt16 *len);
...
void flash_erase_check(void)
{
    ...
    TX_LED = 1;
    RX_LED = 1;
    while (TICK_DIFF(t, get_tick_count()) < 3000)
        if (!SW1)
        {
            TX_LED = 0;
            RX_LED = 0;
...
BOOL eval_save(void)
{
    ...
    if(ONE_NET_PARAM)
    {
        result = dfi_write_segment_of_type(settings_segment_type, (unsigned char *)ONE_NET_PARAM,
          one_net_param_len);
        if (result == (UInt8 *) 0)
        {
            return FALSE;
