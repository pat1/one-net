...
const ona_unit_type_count_t ONE_NET_DEVICE_UNIT_TYPE[ONE_NET_NUM_UNIT_TYPES] = {{ONA_TEMPERATURE_SENSOR, ONE_NET_NUM_UNITS}};

//! the voltage status reading
static UInt8 voltage_status;

//! the temperature reading
static UInt16 g_temperature;
...
*/        
        case ONA_TEMPERATURE:
        {
            ...
            // dst_unit has value 1
            //
            if(ona_parse_temperature(RX_PLD, &src_unit, &dst_unit, &temperature) !=
               ONS_SUCCESS || (dst_unit > ONE_NET_NUM_UNITS) || (dst_unit < 1))
            {
                break;
            } // if parsing the switch payload was not successful //
            switch(msg_class)
            {
                ...
                {
                    // units 1
                    g_temperature = TempTo20thK(read_temperature());
                    if (ona_send_temperature_status(dst_unit, src_unit, g_temperature, SRC_ADDR) == ONS_SUCCESS)
                    {
                        rv = TRUE;
                    } // if sending the status was successful //
                    break;
...
//    get_sw_state(&g_sw);
    ...
    while(1)
    {
        ...
        if (connected)
            if (TICK_DIFF(t_measure, get_tick_count()) > 30000)
            {
                t_measure = get_tick_count();
                g_temperature = TempTo20thK(read_temperature());
                ona_send_temperature_status(0, 0, g_temperature, 0);
            }
//        check_switch();
        one_net_client();
    } // main loop //
